...
///     1. Raycast
///     2. Fixed
///     Raycast casts a raycast from the current position and rotation of the laser until it collides with the player or
///     obstacles.
///     Fixed casts a raycast from the current postion and rotation of the laser, but has a max distance that the laser can
///     travel!
///
///     The laser can also optionally rotate continuously while firing.
/// </summary>
[RequireComponent(typeof(LineRenderer))]
public class MovingDeathLaser : MonoBehaviour
{

    private const int PlayerHierarchyLayerIndex = 3;

    [SerializeField]
    [Tooltip("The sprite to display for the laser emitter when the laser is on")]
    private SpriteRenderer onSprite;
    [SerializeField]
    [Tooltip("The sprite to display for the laser emitter when the laser is off")]
    private SpriteRenderer offSprite;
    [SerializeField]
    [Tooltip("The light to turn on when the layer is on")]
    private Light2D onLight;

    [SerializeField]
    [Tooltip("Raycast: The laser travels until it hits an obstacle.\nFixed: The laser stops at a set distance.")]
    private LaserMode laserMode;
    [SerializeField]
    [Tooltip("What the laser will be able to hit (make sure default and player are always included!)")]
    private LayerMask mask;
    [SerializeField]
    [Tooltip("All the assets that are related to firing the laser.")]
    private Particles particles;
    [SerializeField]
    [Tooltip("All the assets related to Atlas dying.")]
    private Death death;

    [SerializeField]
    [Tooltip("How far the laser will travel before stopping (only for Distance LaserMode!)")]
    private float laserDistance;

    [SerializeField]
    [Tooltip("How smoothly the laser jumps from one position to the next.")]
    [Range(1f, 250f)]
    private float resetSpeed;

    [Header("Rotation")]
    [SerializeField]
    [Tooltip("If true, the laser will continuously rotate while firing.")]
    private bool sweepEnabled = false;
    [SerializeField]
    [Tooltip("How fast the laser rotates, in degrees per second. Positive = counter-clockwise, negative = clockwise.")]
    private float sweepSpeed = 20f;

    private float currentLaserDistance;

    private DeathLaserSound dlsScript;
    private bool laserOn = true; // If the laser is on or off
    private bool lockout; // Death timeout for player
    ...
    private void Update()
    {
        ...
        if (laserOn)
        {
            // Rotate the laser before doing the raycast so the beam reflects the new rotation this frame
            if (sweepEnabled)
            {
                UpdateSweep();
            }
    ...
    /// <summary>
    ///     Continuously rotates the laser in a full circle.
    /// </summary>
    ...
    private void DrawLaser(Vector3 laserTarget)
    {
        ...
        { transform.position, laserTarget });
    ...
    private void CheckForDeath(RaycastHit2D raycastHitData)
    {
        ...
        if (!lockout
            ...
            && !PlayerRef.PlayerManager.GodMode
            && UtilityFunctions.CompareTagOfHierarchy(raycastHitData.collider.transform, tag, out Transform player, PlayerHierarchyLayerIndex))
        {
            // Teleport Atlas
    ...
    private struct Death
    {
        ...
        [Range(1.5f, 10f)]
        [Tooltip("The amount of time before the laser could hit Atlas again!")]
